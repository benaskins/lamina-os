...
class ADRValidator:
        ...
        for field, pattern in self.REQUIRED_METADATA.items():
            ...
            if not match:
                ...
                metadata[field.lower().replace(" ", "_")] = (
                    match.group(1) if match.lastindex else True
                )
        ...
        for section_name, pattern in self.REQUIRED_SECTIONS:
            if not re.search(pattern, content, re.MULTILINE):
                ...
                if "retrospective" in content.lower() and section_name in ["Considered Options"]:
                    warnings.append(
                        f"Missing {section_name} section (may be acceptable for retrospective)"
                    )
                else:
    ...
    def validate_directory(self, directory: Path) -> tuple[list[ValidationResult], dict[str, any]]:
        ...
        # Find all ADR files
        adr_files = sorted(
            [f for f in directory.glob("*.md") if re.match(r"\d{4}-.*\.md$", f.name)]
        )
        ...
        summary = {
            ...
            "missing_breath_first": sum(
                1 for r in results if any("breath-first" in w.lower() for w in r.warnings)
            ),
            "status_distribution": {},
        }
