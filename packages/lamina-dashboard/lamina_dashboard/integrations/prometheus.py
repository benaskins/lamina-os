...
class PrometheusClient:
    ...
    def __init__(self, prometheus_url: str = None):
        ...
        self.session.headers.update({"User-Agent": "Lamina-Sanctuary-Dashboard/1.0"})
    ...
    def get_agent_interactions(self) -> List[Dict[str, Any]]:
        ...
        if result.get("status") == "success":
            for metric in result.get("data", {}).get("result", []):
                ...
                if any(keyword in source.lower() for keyword in agent_keywords) or any(
                    keyword in destination.lower() for keyword in agent_keywords
                ):
                    if len(value) >= 2 and float(value[1]) > 0:
                        interactions.append(
                            {
