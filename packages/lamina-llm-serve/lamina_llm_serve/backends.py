...
BACKENDS = {
    ...
    'mlc-serve': MLCServeBackend,
    'vllm': VLLMBackend
}


def get_backend_for_model(model_info: Dict[str, Any], backend_configs: Dict[str, Any]) -> Optional[LLMBackend]:
    """
    ...
    if not backend_name:
        logger.error(f"No backend specified for model")
        return None
