...
def cmd_suggest_model(manager: ModelManager, args):
    ...
    if suggested:
        ...
        if not manager.is_model_available(suggested):
            print(f"   ⚠️  Model not available on filesystem")
    else:
...
def cmd_download_model(manager: ModelManager, args):
    ...
    if success:
        ...
        if manager.is_model_available(model_name):
            print(f"🎯 Model is now available for use")
        else:
            print(f"⚠️  Model downloaded but may need path configuration in models.yaml")
    else:
...
def cmd_install_model(manager: ModelManager, args):
    ...
    if success:
        ...
        if manager.is_model_available(model_name):
            ...
            print(f"⚠️  Model installed but may need path configuration in models.yaml")
    else:
...
def main():
    ...
    suggest_parser.add_argument('--use-case', choices=['conversational', 'analytical', 'security', 'reasoning'])
    ...
    list_dl_parser.add_argument('--source', choices=['huggingface', 'ollama'], help='Filter by source')
    ...
    download_parser.add_argument('--source', choices=['huggingface', 'ollama'], help='Download source')
